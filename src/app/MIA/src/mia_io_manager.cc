...
GLUI_Panel* MIAIOManager::InitGlui(const GLUI* const glui,
  ...
  prev_image_btn_ =
      new GLUI_Button(image_panel, "Previous Image",
                      UICtrl::UI_PREV_IMAGE_BUTTON, s_gluicallback);
  next_image_btn_ = new GLUI_Button(
      image_panel, "Next Image", UICtrl::UI_NEXT_IMAGE_BUTTON, s_gluicallback);
...
void MIAIOManager::LoadImageToCanvas(PixelBuffer** buffer) {
  IOManager::LoadImageToCanvas(buffer);
...
void MIAIOManager::set_image_file(const std::string& fname_in) {
  ...
  if (!is_valid_image_file_name(image_file)) {
    ...
    std::cout << "MIAIOManager::set_image_file" << std::endl;
    save_file_label()->set_text(
...
std::string MIAIOManager::image_name_plus_seq_offset(
    const std::string& filename, int offset) {
