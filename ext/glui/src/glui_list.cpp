  ...
  if (tmp_line < num_lines) {
    ...
    if (associated_object != NULL)
      if (cb_click_type == GLUI_SINGLE_CLICK) {
        ...
        std::cout << "ATTEMPTING DOUBLE CLICK" << std::endl;
        std::cout << "last line:      "  << last_line << std::endl;
        std::cout << "curr_line line: "  << curr_line << std::endl;
        std::cout << "ms:             "  << ms << std::endl;
        std::cout << "last_click_time:"  << last_click_time << std::endl;
        int diff = (ms - last_click_time + 1000)%1000;
        std::cout << "diff:           " << diff << std::endl;
        if (last_line == curr_line && (diff) < 300) {
          //obj_cb(associated_object, user_id);
          obj_cb(this);
        } else {
          last_click_time = ms;
